...
func NewRedisService(name string, cfg *redis.Config) (ser redis.IService, err error) {
	if cfg == nil {
		err = errors.New("组件 [redis] [" + name + "] 配置为空，请检查配置")
		framework.Error(err.Error())
		return
	}

	if cfg.Disabled {
		framework.Warn("组件 [redis] [" + name + "] 已禁用，不创建")
		return
	}

	// 创建 服务
	ser, err = redis.New(cfg)
	if err != nil {
		err = errors.New("组件 [redis] [" + name + "] 创建 失败: " + err.Error())
		framework.Error(err.Error())
		return
	}
