...
func TableManageRolePermissionCreate(dbService db.IService) (err error) {
	var version = "v1.0.0"
	var tableName = "manage_role_permission"
	var moduleName = "manage"
	// 定义表结构
	table := &db.Table{
		Name:    tableName,
		Comment: "管理后台 角色 权限",
		ColumnList: []*db.Column{
			{Name: "role_permission_id", DataType: "bigint", Length: 20, NotNull: true, Key: true, Comment: "角色 权限 ID"},
			{Name: "role_id", DataType: "bigint", Length: 20, NotNull: true, Comment: "角色 ID"},
			{Name: "permission", DataType: "varchar", Length: 1000, Comment: "权限 如：/admin/user/add"},
			{Name: "create_at", DataType: "bigint", Length: 20, Comment: "创建 时间戳 毫秒"},
			{Name: "update_at", DataType: "bigint", Length: 20, Comment: "修改 时间戳 毫秒"},
		},
		IndexList: []*db.Index{
